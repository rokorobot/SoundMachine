...
export default function SoundMachinaApp() {
  ...
  // Initial prompt generation when blueprint loads
  useEffect(() => {
    generatePrompts();
  }, [blueprint.genre, blueprint.bpm, blueprint.target_model]);

  const activePreset = presets.find(p => p.id === selectedPresetId);
  const activePresetName = activePreset ? activePreset.name : 'CUSTOM CONFIG';
